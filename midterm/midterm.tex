...
\begin{enumerate}[label=(\roman*)]
    ...
    \begin{enumerate}[label=(\alph*)]
        ...
        \begin{gather*}
            P(X > t) = P\big(\exp(\lambda X) > \exp(\lambda t)\big) \le \frac{\mathbb{E}[\exp(\lambda X)]}{\exp(\lambda t)}
        \end{gather*}
...
\begin{enumerate}[label=(\roman*)]
    ...
    Let $F_0 $ be the CDF of $T(X)$ under $\theta_0 $. By continuity, $U := F_0\big(T(X)\big) \sim U(0, 1)$ (probability integral transform \cite[Appendix B, \textsc{Theorem} 1]{appendix}). Let $X'$ be an independent copy of $X $ under $H_0 $. Then 
    \begin{gather*}
        p(X) = P_{\theta_0}\big(T(X') \ge T(X) | X\big) = 1 - F_0\big(T(X)\big) = 1 - U
    \end{gather*}
    because $T(X') \sim F_0 $ and continuity implies $P\big(T(X')\ge t\big) = 1 - F_0(t)$. Hence, for any $\alpha \in [0, 1]$,
    \begin{gather*}
        P_{\theta_0}\big(p(X) \le \alpha\big) = P(1 - U \le \alpha) = P(U \ge 1 - \alpha) = \alpha,
    \end{gather*}
    so $p $ is valid.
    ...
    We take the two-sided statistic $T(X) = |X| $. Under $H_0, Z := X \sim \mathcal{N}(0, 1)$. By the definition $p(x) = P_0\big(T(Z) \ge T(x)\big)$, using $T(X) = |X|$ (and equivalently, $T(X) = X^2 $):
    \begin{gather*}
